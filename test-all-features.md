...
- [ ] `🗑 削除` … 確認ダイアログののち、ブロックを削除（Undo で復元可）
...
本文中の数式は KaTeX でレンダリングされます。`$...$` がインライン、`$$...$$` がディスプレイ数式です。
...
- [ ] 上記すべての式が崩れず描画される（赤いエラー表示が出ない）
- [ ] 段落をダブルクリック編集 → 数式ソースが `$...$` / `$$...$$` のまま編集できる
- [ ] 編集確定後、再描画される
- [ ] コードブロック内（``` ``` ```内）の `$...$` は数式化されない
...
一時 HTML が生成され、OS 既定のブラウザで開かれて自動的に印刷ダイアログが起動します。「PDF として保存」を選択して保存先を指定してください。
