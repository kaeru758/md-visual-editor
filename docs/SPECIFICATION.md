...
`Ctrl+C` は**テキスト選択がある場合は横取りしない**（ブラウザ既定のコピーが `text/plain` と `text/html` の両方を載せるため）。選択が無いときのみブロック単位のコピーとして動作する。

`Ctrl+V` は `preventDefault()` **しない**（v1.1.5〜）。ブラウザ既定の `paste` イベントを発火させ、`clipboardData` から `text/html` を読むことで PowerPoint / Excel の表を取得する（`clipboard.readText()` は `text/plain` しか見えない）。

ただし **Chromium は編集可能な要素が貼り付け対象のときしか `paste` イベントを発火しない**。ブロック選択は編集可能ではないため、`Ctrl+V` の keydown 内で画面外の 1px `contenteditable`（`.paste-capture`）へ同期的にフォーカスを移し、それを貼り付けの受け皿にする（v1.1.5〜）。イベント処理後にフォーカスは元の要素へ戻す。`clipboardData` には対象要素に関わらず全形式が載るため、これで `text/html` が読める。

`paste` イベントが届かないホスト向けに `setTimeout(0)` のフォールバックがあり、`navigator.clipboard.read()`（**1.5 秒でタイムアウト**。webview では権限確認が返らずハングし得るため）→ `readText()` の順で読み直す。2 経路が同時に成立して二重挿入にならないよう、`Ctrl+V` ごとに発行するトークンで排他する。
...
| `Shift+Click` | 範囲選択 |
| 背景クリック | 選択解除 |
| コピー/切り取り | 選択ブロックの raw を `\n\n` で連結して `text/plain` へ。同時に**描画結果の HTML を `text/html` へ**書き込む（v1.1.5〜）。貼り付け先が形式を選ぶため、PowerPoint / Word / Excel には表として、テキストエディタには Markdown ソースとして貼り付く。テキスト選択がある場合は横取りせずブラウザ既定のコピーに任せる |
| 貼り付け | クリップボードの内容を `marked.lexer()` で再解析してトークンとして挿入。**挿入位置は選択ブロックの直後**（`rangeOf(start).end`。v1.1.4 以前は `start + 1` でセクションの見出し直後＝ブロック内部に入っていた）。前後に空行が無い場合は `space` トークンで補い、隣接ブロックとの結合を防ぐ |
| 貼り付け（外部アプリから） | `text/html` があれば HTML → Markdown 変換して挿入（v1.1.5〜）。PowerPoint / Excel / Word の表は GFM テーブルに、見出し・箇条書き・リンク・太字も変換。`text/html` が無く TSV 形状の `text/plain` のみの場合はテーブル化。自前のコピー（HTML 内のマーカーで判別）は `text/plain` の Markdown ソースをそのまま使用 |
