...
- **ブロックの削除** — 右クリックメニューの「🗑 削除」または `Delete` / `Shift+Delete` キーで削除（削除前に内容を自動的にクリップボードへコピーし、誤操作からの復旧を支援）。
- **画像表示** — `![alt](path.png)` の相対パスは Markdown ファイルのディレクトリを基準にホスト拡張側で解決し、WebView の CSP 制約下でも表示。`file://` 絶対パスや `data:` URL も対応。ファイル／フォルダから画像をブロック上にドラッグ & ドロップして挿入することも可能。
...
```powershell
code --install-extension md-visual-editor-0.5.1.vsix
```
