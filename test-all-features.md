...
- [ ] SVG 上でノードを右クリック → `🎨 色を変更` でノードごとの背景色・文字色を設定できる（`style` ディレクティブとして保存され、再オープンで復元される）
- [ ] 方向切り替え `TB / LR / RL / BT`
...
- [ ] **【v1.0.0】SVG 上のクラスをダブルクリックすると、そのクラス名の編集が始まる**
- [ ] **【v1.0.0】ツールバーの「🎨 色」からクラスごとの背景色・文字色を変更できる（`classDef` として保存され、再オープンで復元される）**
...
- [ ] **【v1.0.0】SVG 上のエンティティをクリックすると選択され、左パネルで属性を編集できる**
...
> **【v1.0.0】新機能:**
> - **図のノード直接クリック編集** — 対応図種（状態遷移図 8.1 / パイチャート 8.2 / ユーザージャーニー 8.3 / 要求図 8.6 / C4 図 8.7 / ブロック図 8.10 / アーキテクチャ図 8.13 / Kanban 8.14）では、プレビュー中の図のノードをクリックすると左パネルの該当項目までスクロール＋ハイライトし、その項目の編集が始まります。
> - **要素の背景色・文字色（🎨 色）** — 状態遷移図 8.1 / C4 図 8.7 / ブロック図 8.10 では、ツールバーの「🎨 色」から要素ごとの背景色・文字色を設定できます（状態遷移図・ブロック図は `classDef`、C4 図は `UpdateElementStyle` ディレクティブとして保存され、コードとラウンドトリップします）。
...
ZenUML レンダラーはバンドルに含まれないため、挿入ピッカーには表示されず、既存のコードブロックも**コード編集のみ**動作します（プレビューは描画されないか、フォールバック表示になります。§17 参照）。
...
- [ ] 上のブロックは編集ボタンからコード編集でき、プレビューが出ない場合でもエディタ全体は壊れない
...
- [ ] **【v1.0.0】** 対応図種（8.1 / 8.2 / 8.3 / 8.6 / 8.7 / 8.10 / 8.13 / 8.14）でプレビュー上のノードをクリック → 左パネルの該当項目へスクロール＋ハイライトし、編集が始まる
- [ ] **【v1.0.0】** 状態遷移図 8.1 / C4 図 8.7 / ブロック図 8.10 でツールバー「🎨 色」→ 要素の背景色・文字色を変更でき、保存後（`classDef` / `UpdateElementStyle`）に再オープンしても色が復元される
...
## 19. 【v1.0.0】新機能・修正の回帰チェック

v1.0.0 で追加・修正された挙動をまとめて確認します。個々の操作手順は各セクションを参照してください。

### 19.1 図のノード直接クリック編集（→ §7・§8）

- [ ] フォーム系エディタ（状態遷移図・パイチャート・ユーザージャーニー・要求図・C4 図・ブロック図・アーキテクチャ図・Kanban）で、プレビュー中のノードをクリック → 左パネルの該当項目へスクロール＋ハイライトし編集が始まる
- [ ] クラス図はダブルクリックで編集、ER 図はクリックで選択できる

### 19.2 要素の背景色・文字色（🎨）（→ §7・§8）

- [ ] クラス図・状態遷移図・ブロック図・C4 図のツールバー「🎨 色」から要素ごとに背景色・文字色を設定できる
- [ ] フローチャートは SVG ノードの右クリック「🎨 色を変更」から設定できる
- [ ] いずれも保存すると `style` / `classDef` / `UpdateElementStyle` としてコードに書き出され、再オープンで色が復元される（ラウンドトリップ）

### 19.3 ディスプレイ数式 `$$…$$` の独立ブロック化

- [ ] H1/H2 セクションの本文中に `$$…$$` のディスプレイ数式を置くと、前後の文章とは**別の独立ブロック**として扱われる（表・Mermaid と同様）
- [ ] 段落中のインライン数式 `$…$` は従来どおり文章の一部のまま（独立ブロックにはならない）

### 19.4 無編集での離脱がタブを「未保存」にしない

- [ ] 任意のブロックをダブルクリックで編集モードに入り、**何も変更せずに** `Escape`（または `Ctrl+Enter`）で抜ける → VS Code タブが「未保存（●）」にならない
- [ ] 表・Mermaid エディタを開いて何も変えずに閉じても同様に未保存にならない

### 19.5 PDF 出力プレビューのハイライト残り修正

- [ ] ブロックを右クリックして選択状態のまま `📄 PDF として出力` → 既定ブラウザのプレビュー HTML で、選択ブロックの背景ハイライトが**残らない**（ドラッグ中・画像ドロップ中の一時クラスも紙面に出ない）
